...
using Semana10_CodeFirst.Models;

namespace Semana10_CodeFirst.Controllers
{
    public class OfficeAssignmentsController : Controller
    {
        ...
        {
            var officeAssignments = db.OfficeAssignments.Where(x => x.Active == true).Include(o => o.Instructor);
        ...
        public ActionResult Create([Bind(Include = "InstructorID,Active,Location")] OfficeAssignment officeAssignment)
        {
            if (ModelState.IsValid)
            {
                officeAssignment.Active = true;
        ...
        public ActionResult Edit([Bind(Include = "InstructorID,Location")] OfficeAssignment officeAssignment)
        {
            if (ModelState.IsValid)
            {
                OfficeAssignment officeAssFound = db.OfficeAssignments.Find(officeAssignment.InstructorID);
                officeAssFound.Location = officeAssignment.Location;
                db.Entry(officeAssFound).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            ViewBag.InstructorID = new SelectList(db.Instructors, "ID", "LastName", officeAssignment.InstructorID);
